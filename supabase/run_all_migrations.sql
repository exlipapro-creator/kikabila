-- =================================================================
-- KIKABILA — Complete schema, safe to run on a fresh Supabase project
...
-- Add columns if upgrading from older migration
...
BEGIN
  ...
  FROM public.submissions WHERE user_id = NEW.user_id AND created_at::date = CURRENT_DATE;
  ...
  IF NEW.cultural_note IS NOT NULL AND length(btrim(NEW.cultural_note)) > 0 THEN
    gained := gained + 5; reason := reason || ' + cultural note';
  END IF;
  ...
  FROM public.submissions WHERE user_id = NEW.user_id AND agreed_with_consensus IS NOT NULL;
...
BEGIN
  ...
  UPDATE public.profiles SET freeze_tokens = freeze_tokens - 1, last_played_on = CURRENT_DATE - 1
  WHERE id = auth.uid();
  RETURN true;
END; $$;
...
-- =================================================================
-- DONE. Run this once in SQL Editor — your database is fully ready.
-- =================================================================
